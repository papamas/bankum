		...
		<div class="pd-ltr-20">
		   ...
		   <div class="col-sm-12 col-md-12 mb-30">
				<div class="card card-box">
					<form action="<?php echo site_url()?>/perkara/putusanSimpan" method="post" enctype="multipart/form-data">
					<div class="card-header">
						Tahapan Perkara Putusan/Penetapan
					</div>
					<div class="card-body">
                        <?php echo $pesan;?>					
						<div class="form-group row">
							<input type="hidden" name="tahapanPerkara" value="<?php echo $perkara->tahapan?>" />
							<input type="hidden" name="aksi" value="<?php echo (!empty($perkara->tanggal_putusan) ?  'update' : 'new') ?>" />	
						...
						<hr/>
						<div class="form-group row">
							<label class="col-md-2 col-form-label">Tanggal Putusan</label>
							<div class="col-md-4">
								<input name="tanggalPutusan" value="<?php echo (!empty($perkara->tanggal_putusan) ?  $perkara->tanggal_putusan : set_value('tanggalPutusan') ); ?>" class="form-control date-picker" type="text" placeholder="Tanggal Putusan">
								<?php echo form_error('tanggalPutusan'); ?>
							</div>
							<label class="col-md-3 col-form-label">Tanggal Pemberitahuan</label>
							<div class="col-md-3">
								<input name="tanggalPemberitahuan" value="<?php echo (!empty($perkara->tanggal_pemberitahuan) ?  $perkara->tanggal_pemberitahuan : set_value('tanggalPemberitahuan') ); ?>" class="form-control date-picker" type="text" placeholder="Tgl Pemberitahuan">
								<?php echo form_error('tanggalPemberitahuan'); ?>
							</div>
						</div>	
						
						<div class="form-group row">
							<label class="col-md-2 col-form-label">Tanggal Diterima</label>
							<div class="col-md-10">
								<input name="tanggalDiterima" value="<?php echo (!empty($perkara->tanggal_diterima) ?  $perkara->tanggal_diterima : set_value('tanggalDiterima') ); ?>" class="form-control date-picker" type="text" placeholder="Tanggal Putusan">
								<?php echo form_error('tanggalDiterima'); ?>
							</div>								
						</div>
						<div class="form-group row">
						...
						<div class="form-group row">
							<label class="col-md-2 col-form-label">Amar Putusan</label>
							<div class="col-md-10">
								<textarea name="amarPutusan" rows="2" class="form-control"><?php echo (!empty($perkara->amar_putusan) ?  $perkara->amar_putusan : set_value('amarPutusan') ); ?></textarea>
								<?php echo form_error('amarPutusan'); ?>
							</div>								
						</div>						
						<div class="form-group row">
							<label class="col-md-2 col-form-label">Incracht</label>
							<div class="col-md-4">
							    <input type="radio" value="1" name="inkracht"  <?php echo ($perkara->inkracht == 1 ?  'checked' : set_radio('inkracht', 1)) ; ?>/>&nbsp;Ya
								<input type="radio" value="2" name="inkracht" <?php echo ($perkara->inkracht == 2 ?  'checked' : set_radio('inkracht', 2)) ; ?>/>&nbsp;Tidak
								<?php echo form_error('inkracht'); ?>	
							</div>						
							<label class="col-md-2 col-form-label">File scan</label>
							<div class="col-md-4">
								<input type="file" name="fileScan" class="form-control" />
								<a style="color:blue;" class="<?php echo (!empty($perkara->tanggal_putusan) ?  '' : 'd-none') ?>" data-toggle="modal"  data-tooltip="tooltip" data-placement="top" title="Lihat Surat" data-target="#fileSurat" data-id="?f=<?php echo $this->myencrypt->encode($perkara->file_scan)?>"><i class="fa fa-eye"> Lihat File</i></a>
							</div>							
						</div>
						<div class="<?php echo (!empty($perkara->tanggal_putusan) ?  '' : 'd-none') ?> form-group row">
