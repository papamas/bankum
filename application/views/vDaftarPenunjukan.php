...
<body>
			...
			<!-- Responsive tables Start -->
				<div class="pd-20 card-box mb-30">
					<div class="clearfix mb-20">						
							<h4 class="text-blue h4">Daftar Surat Perintah Penunjukan Saksi/Ahli</h4>	
							 <?php echo $pesan; ?>
							<form name="frmDaftar" action="<?php echo site_url()?>/penunjukan/daftar" method="post">
						...
						<table id="tbDaftar"  class="table table-striped">
							...
							<tbody>
								...
								<tr>
									...
									<td>
									<a href="#" style="color:blue;"  data-id="<?php echo $this->myencrypt->encode($value->nomor)?>" data-toggle="modal" data-target="#ePenunjukan" data-placement="top"  title="Edit"><i class="fa fa-pencil"></i></a>
									...
									<a style="color:red;"  data-toggle="modal"  data-tooltip="tooltip" data-placement="top" title="Lihat File Surat" data-target="#fileSurat" data-id="?f=<?php echo $this->myencrypt->encode($value->file_surat)?>"><i class="fa fa-eye"></i></a>
									<a style="color:blue;"  data-nomor="<?php echo $this->myencrypt->encode($value->nomor) ?>" data-tooltip="tooltip" data-placement="top"  data-toggle="modal" data-target="#kirimModal" title="Kirim untuk approval"><i class="fa fa-mail-forward"></i></a>		</td>
								</tr>	
								<?php $no++; endforeach?>	
							</tbody>
						</table>
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
				    <h5 class="modal-title"><span id="msg"></span></h5>
	...
	<div class="modal" id="fileSurat" tabindex="-1" role="dialog" aria-hidden="true">
	    <div class="modal-dialog  md-dialog modal-lg">
		  <div class="modal-content md-content">
				<div class="modal-header">
					<h4 class="modal-title" >File Surat Penunjukan</h4>
					<button type="button" class="close" data-dismiss="modal">&times;</button>					
				</div>	
	...
	<div id="ePenunjukan" class="modal fade" role="dialog">
		<div class="modal-dialog modal-lg"" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">								
					<form id="efrmpenunjukan" method='post' method='post' enctype="multipart/form-data">				
	...
	$(document).ready(function () {
		...
		$('#uploadModal').on('show.bs.modal',function(e){
			$('#uploadModal #msg').text('Upload File Surat Penunjukan')
		...
		$('#btn_upload').click(function(){
			...
			// AJAX request
			$.ajax({
				url: '<?php  echo site_url()?>/penunjukan/doUpload',
				type: 'post',
				data: data,
				contentType: false,
				processData: false,
		...
		$('#fileSurat').on('show.bs.modal',function(e) {    		
			...
			var iframe = $('#frame');
			iframe.attr('src', '<?php echo site_url()?>'+'/penunjukan/showFile/'+id);			
	    });
		...
		$("#nBtnKirim").on("click",function(e){
			...
			$.ajax({
				type: "POST",
				url : "<?php echo site_url()?>/penunjukan/kirimApproval",
				data: data,
				success: function(r){
		...
		$("#ePenunjukan").on("show.bs.modal",function(e){
			$('#ePenunjukan #msg').text('Edit Data Surat Perintah Penunjukan') 
			...
			$.ajax({
				url: "<?php echo site_url()?>/penunjukan/getpenunjukanByid",
				dataType:'json',
				type:'GET',
				data:{nomor:id},
				success: function(r){                         
					$('#ePenunjukan input[name=nomor]').val(r.nomor);
					$('#ePenunjukan input[name=dasarSatu]').val(r.dasar_satu);
					$('#ePenunjukan input[name=dasarDua]').val(r.dasar_dua);	
					$('#ePenunjukan input[name=kepada]').val(r.kepada);
					$('#ePenunjukan [name=untuk]').val(r.untuk);
					$('#ePenunjukan input[name=diKeluarkan]').val(r.di_keluarkan);
					$('#ePenunjukan input[name=tanggalSurat]').val(r.tanggal_surat);
					$('#ePenunjukan [name=tertanda]').val(r.tertanda);					
					$('#ePenunjukan input[name=nomorOld]').val(r.nomor);					
				}				
			});		
		});
	
		$("#eBtn").on("click",function(e){                     
			$('#ePenunjukan #msg').text('Updating Please Wait.....')
				  .removeClass( "text-success")
				  .removeClass("text-danger")
				  .addClass( "text-info" );
			 
			var form = $('#efrmpenunjukan')[0];
			var data = new FormData(form);		
			$.ajax({
				url: '<?php  echo site_url()?>/penunjukan/update',
				type: 'post',
				data: data,
				contentType: false,
				processData: false,
				...
				error : function(r){
					$('#ePenunjukan #msg').text(r.responseJSON.pesan)
						 .removeClass( "text-info")							 
						 .removeClass( "text-success")
		...
		$('#pratinjauSurat').on('show.bs.modal',function(e) { 			
			...
			iframe.attr('src', '<?php echo site_url()?>'+'/penunjukan/pratinjauSurat/'+ id);			
	    });
		
		function refreshTable(){						
			$.ajax({   
			    type: 'POST',   
			    url: '<?php echo site_url()?>/penunjukan/getpenunjukan',   
