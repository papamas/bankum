...
class Penunjukan extends MY_Controller {
	...
	function __construct()
    {
		...
		$this->load->model('mPenunjukan');
    }
	
	public function index()
	{
		$data['pesan']			= '';
		$this->load->view('vPenunjukan',$data);
	}
	...
	public function daftar()
	{
		...
		$data['penunjukan']			= $this->mPenunjukan->getSuratPenunjukan();
		$this->load->view('vDaftarPenunjukan',$data);
	}
	...
	public function simpan()
	{
		...
		{	
				$dataPenunjukan['nomor']				= $this->input->post('nomor');
				$dataPenunjukan['dasar_satu']			= $this->input->post('dasarSatu');
				$dataPenunjukan['dasar_dua']			= $this->input->post('dasarDua');
				$dataPenunjukan['kepada']				= $this->input->post('kepada');
				$dataPenunjukan['untuk']				= $this->input->post('untuk');
				$dataPenunjukan['di_keluarkan']			= $this->input->post('diKeluarkan');
				$dataPenunjukan['tanggal_surat']		= date('Y-m-d',strtotime($this->input->post('tanggalSurat')));	
				$dataPenunjukan['tertanda']				= $this->input->post('tertanda');
				$dataPenunjukan['jenis_surat']			= 'penunjukan';
				$dataPenunjukan['tahapan']			= '1';
		
				$db_debug 			= $this->db->db_debug; 
				$this->db->db_debug = FALSE; 	
				if (!$this->mPenunjukan->simpan($dataPenunjukan)) 
				{
					$error = $this->db->error();			
					if(!empty($error))
					{
				...
				{
					$data['response']		= TRUE;
					$data['pesan']			= '<div class="alert alert-success">Berhasil menyimpan surat penunjukan</div>';    
	...
	public function doUpload()
	{
		...
		else
		{
				...
				$dataPenunjukan['file_surat']		= $fileupload['file_name'];
			
				$db_debug 			= $this->db->db_debug; 
				$this->db->db_debug = FALSE; 	
				if (!$this->mPenunjukan->updateSuratPenunjukan($dataPenunjukan)) 
				{
					$error = $this->db->error();			
					if(!empty($error))
					{
	...
	public function showFile()
	{
		...
		$flok      = base_url().'uploads/penunjukan/'.$file;
	...
	public function kirimApproval()
	{
		...
		$db_debug 			= $this->db->db_debug; 
		$this->db->db_debug = FALSE; 	
		if (!$this->mPenunjukan->updateTahapanSuratPenunjukan($nomor)) 
		{
			$error = $this->db->error();			
			if(!empty($error))
			{
	...
	function getPenunjukanByid()
	{
		$nomor  		= $this->myencrypt->decode($this->input->get('nomor'));
		$penunjukan		= $this->mPenunjukan->getPenunjukanByid($nomor)->row();
		echo json_encode($penunjukan);
	}	
	...
	public function update()
	{
		...
		{			    		
			...
			if (!$this->mPenunjukan->update($dataPenunjukan)) 
			{
				$error = $this->db->error();			
				if(!empty($error))
				{
	...
	{
		$penunjukan 		= $this->mPenunjukan->getSuratPenunjukan();
		$no 				= 1;
		...
		foreach($penunjukan->result() as $value)
		{
			$html .='<tr>
				...
				<td>'.$value->untuk.'</td>
				<td>
				<a href="#" style="color:blue;"  data-id="'.$this->myencrypt->encode($value->nomor).'" data-toggle="modal" data-target="#ePenunjukan" data-placement="top"  title="Edit"><i class="fa fa-pencil"></i></a>
	...
	public function pratinjauSurat()
	{
		...
		$this->pdf->Line(72, 60, $this->pdf->getPageWidth() - 77, 60, $style1);
		$this->pdf->Text(5, 60,'Nomor : '.$penunjukan->nomor , false, false, true, 0, 4, 'C', false, '', 0, false, 'T', 'M', false);
		...
		$this->pdf->Text(27, 100, '3.');
		$text1='Surat Kepala Kepolisian Resort Tangerang Selatan Nomor B/383/II/2019/Reskrim tanggal 7 Februari 2019 perihal  Bantuan menghadirkan saksi;';
		$this->pdf->writeHTMLCell(160,125,35,100,$penunjukan->dasar_satu,0,0,false,true,'J',true);
		
		$this->pdf->Text(27, 110, '4.');
		$text1='Disposisi Kepala Badan Kepegawaian Negara tanggal 15 Februari 2019 untuk ditindaklanjuti;';
		$this->pdf->writeHTMLCell(160,125,35,110,$penunjukan->dasar_dua,0,0,false,true,'J',true);
		...
		$this->pdf->Text(25, 130, ':');
		$text1='Yuyud Yuchi Susanta, S.H. NIP. 196410011991031001, Pangkat Pembina Tingkat I, golongan ruang IV/b, Jabatan Kepala Bidang Konsultasi Hukum Kepegawaian pada Badan Kepegawaian Negara';
		$this->pdf->writeHTMLCell(170,125,27,130,$penunjukan->kepada,0,0,false,true,'J',true);
		
		$this->pdf->Text(5, 150, 'Untuk');
		$this->pdf->Text(25, 150, ':');
		$this->pdf->Text(27, 150, '1.');
		$text1='Memberikan keterangan klarifikasi tentang Surat penetapan NIP kepada Penyidik Polres Tangerang Selatan terkait dengan adanya dugaan tindak pidana penipuan sebagaimana dimaksud dalam Pasal 378 KUHP dan menurut keterangan korban bahwa pelaku menjanjikan bisa menjadikan anak korban yang bernama ARIE CAHYANI EKA PUTRI untuk menjadi Pegawai di Dinas Kesehatan Pemkot tangerang Selatan dan kemudian pelaku memberikan kepada korban NIP dengan Nomor 1999501012017042003 an. ARIE CAHYANI EKA PUTRI, yang akan dilaksanakan pada hari Selasa tanggal 12 Maret 2019; ';
		$this->pdf->writeHTMLCell(160,125,35,150,$penunjukan->untuk,0,0,false,true,'J',true);
		...
		$this->pdf->Text(120, 210, 'Dikeluarkan');
		$this->pdf->Text(145, 210, ': '.$penunjukan->di_keluarkan);
		
		$this->pdf->Text(120, 215, 'pada tanggal');
		$this->pdf->Text(145, 215, ': '.$this->tgl_indo($penunjukan->tanggal_surat));
